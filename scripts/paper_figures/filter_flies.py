...
class FilterData:
    ...
    def apply_mask_to_group(group_data):
        ...
        for column in binary_masks_df.columns:
            masked_data_df[column] = masked_data_df[column].where(binary_masks_df[column], other=None)
    ...
    def apply_binary_masks(binary_masks_df, data_df, n_workers=None):
        ...
        groups = [(binary_masks_groups.get_group(name), data_groups.get_group(name)) for name in
                  binary_masks_groups.groups.keys()]
    ...
    @staticmethod
    def median_filter_resample(df, filter_size, resampling_ratio, sampling_rate, n_workers=None):
        # Prepare inputs for parallel processing
        inputs = [(col, df[col], filter_size, resampling_ratio) for col in df.columns if col != 'ExptNames']

        # Apply median filter and resample in parallel
        with ProcessPoolExecutor(max_workers=n_workers) as executor:
            resampled_columns = list(executor.map(FilterData._apply_median_filter_resample, inputs))

        # Create a new DataFrame with resampled data
        resampled_df = pd.DataFrame(dict(resampled_columns))
        resampled_df.index = pd.to_timedelta(np.arange(0, len(resampled_df)) / sampling_rate, unit='s')
